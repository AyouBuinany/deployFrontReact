...
export const validateNumberPhone = new Validation(
  'input should be an Number phone',
  text => !!text.match(/[0-9]]+/i),
  'invalid Number Phone'
)
export const validateLength = (min_len, max_len,name) => {
  const regex = new RegExp(`(?=.{${min_len},${max_len}})`, 'g')
  return new Validation(
    'input should within certain length',
    text => !!text.match(regex),
    `${name} has to be ${min_len}-${max_len} letter`
  )
}
